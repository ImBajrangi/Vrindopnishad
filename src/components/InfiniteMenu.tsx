...
class InfiniteGridMenu {
  ...
  private render(): void {
    ...
    gl.uniform4f(
      ...
      this.smoothRotationVelocity * 0.4
    );
...
export const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      if (newItem) {
        setActiveItem(prev => (prev?.link === newItem.link && prev?.title === newItem.title ? prev : newItem));
      }
    };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '600px', borderRadius: '24px', overflow: 'hidden' }}>
      <canvas id="infinite-grid-menu-canvas" ref={canvasRef} style={{ borderRadius: '24px' }} />
