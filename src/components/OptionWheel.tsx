...
const OptionWheel = ({
  ...
  const runFrame = useCallback((now: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      el.style.transform = `translate3d(${x.toFixed(2)}px, calc(${y.toFixed(2)}px - 50%), 0px) rotate(${rot.toFixed(3)}deg)`;
  ...
  useEffect(() => {
    ...
    // Force rAF layout passes to guarantee DOM item refs receive transforms once mounted
    const forceLayout = () => {
      lastRef.current = performance.now();
      runFrame(performance.now());
    };
    forceLayout();
    const t1 = setTimeout(forceLayout, 16);
    const t2 = setTimeout(forceLayout, 60);
    const t3 = setTimeout(forceLayout, 150);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
      clearTimeout(t3);
