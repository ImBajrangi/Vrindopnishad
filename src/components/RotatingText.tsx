...
export type RotatingTextProps = {
  ...
  // Optional styling & compatibility props
...
export function RotatingText({
  ...
  const activeIntervalMs = rotationInterval ?? ROTATION_INTERVAL_MS;
  ...
  }, [auto, safeTexts.length, staggerFrom, transition, activeIntervalMs]);
  ...
  const effectiveBadgeBg = (style?.background || style?.backgroundColor || badgeBackground) as React.CSSProperties['backgroundColor'];
  const effectiveTextColor = (style?.color || color) as React.CSSProperties['color'];
  const effectiveRadius = style?.borderRadius ?? badgeRadius;
