...
export const generateTextTargets = (
  ...
  const glyphs = characters.map((ch) => GLYPHS[ch] ?? GLYPHS[ch.toLowerCase()] ?? FALLBACK_GLYPH);
...
const pickScatterDestination = (
  ...
  if (target.y <= band.cy) gy -= 0.15;
  else gy += 0.15;

  // Minimized scatter strength (tight, subtle dispersion)
  const spreadX = Math.max(2, markH * 0.8);
  const spreadY = Math.max(2, markH * 1.0);

  let dx = gx * spreadX;
  let dy = gy * spreadY;

  const maxRx = spreadX * 1.3;
  const maxRy = Math.min(band.height * 0.25, spreadY * 1.4);
  const nx = dx / maxRx;
  const ny = dy / maxRy;
...
const GlyphMark = ({ target, index, scattered, band, color, reducedMotion }: GlyphMarkProps) => {
  ...
  const w = (isDot ? target.s : target.w) * 1.85;
  const h = target.s * 1.85;
  const radius = h / 2;
