...
export default function LEDTicker({
  ...
  useEffect(() => {
    ...
    function measure() {
      ...
      const fitCellFromH = (canvas.height * 0.9) / boardRows;
      const fitCellFromW = (canvas.width * 0.98) / Math.max(1, total);
      cell = Math.max(2, Math.min(fitCellFromH, fitCellFromW));
      radius = cell * 0.44;
      visibleCols = Math.max(total, Math.ceil(canvas.width / cell));
      yPad = (canvas.height - boardRows * cell) / 2;
      rowSource = [0, 1, 2, 3, 4, 5, 6];
    }
    ...
    function draw(time: number) {
      ...
      const xBase = isStatic ? 0 : -sign * shift;

      if (!isStatic && offColor && offColor !== "transparent") {
        ctx.beginPath();
      ...
      ctx.beginPath();
      const numColsToDraw = isStatic ? total : visibleCols;
      for (let cx = 0; cx < numColsToDraw; cx++) {
        const source = isStatic ? cx : Math.floor((cx + sign * whole) / scale);
