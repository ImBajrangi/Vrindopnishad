...
// MAIN COMPONENT
// ------------------------------------------------------------ //
export function VaporizeTextCycle(props: VaporizeTextCycleProps = {}) {
    ...
    const fontConfig = useMemo(() => {
        const fontSize = parseNumericFontSize(font.fontSize);
...
function parseNumericFontSize(val: any): number {
    if (typeof val === "number") return isNaN(val) ? 48 : val;
...
const renderCanvas = ({
    ...
    const fontSize = parseNumericFontSize(framerProps.font?.fontSize);
    ...
    const driftRoom =
        ...
        DRIFT_REACH *
        0.6;
    const bleed = Math.ceil(Math.min(400, (isNaN(overflowX) ? 0 : overflowX) + fontSize + (isNaN(driftRoom) ? 0 : driftRoom)));

    const cssW = Math.max(10, Math.floor(width + bleed * 2));
    const cssH = Math.max(10, Math.floor(height + bleed * 2));

    if (isNaN(cssW) || isNaN(cssH)) return;
    ...
    const targetW = Math.floor(cssW * globalDpr);
    const targetH = Math.floor(cssH * globalDpr);
...
const createParticles = (
    ...
    const styleW = parseFloat(canvas.style.width) || canvas.width || 1;
    const calcDPR = (canvas.width || 1) / styleW;
    const sampleRate = Math.max(1, Math.round(isNaN(calcDPR) ? globalDpr : calcDPR));
